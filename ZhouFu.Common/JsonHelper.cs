                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    for (int j = 0; j < dt.Columns.Count; j++)
                    {
                        if (j < dt.Columns.Count - 1)
                        {
                            ...
                            {
                                JsonString.Append("\""+dt.Columns[j].ColumnName.Trim() + "\":" +dt.Rows[i][j].ToString() + ",");
                            }
                            else if (dt.Columns[j].DataType == typeof(string))
                            {
                                JsonString.Append("\"" + dt.Columns[j].ColumnName.Trim() + "\":" + "\"" + dt.Rows[i][j].ToString().Trim(new char[] { ' ' }).Replace("\"", "\\\"") + "\",");
                            }
                            else
                            {
                                JsonString.Append("\"" + dt.Columns[j].ColumnName.Trim() + "\":" + "\"" + dt.Rows[i][j] + "\",");
                            }
                        }
                        else if (j == dt.Columns.Count - 1)
                        {
