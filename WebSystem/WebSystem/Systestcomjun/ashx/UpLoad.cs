...
namespace WebSystem.tools
{
    public class UpLoad
    {
        private ZhongLi.Model.siteconfig siteConfig;

        public UpLoad()
        {
            siteConfig = new ZhongLi.Bll.siteconfig().loadConfig(Utils.GetXmlMapPath("Configpath"));
        }
        #region
        /// <summary>
        /// 裁剪图片并保存
        ...
        private bool IsImage(string _fileExt)
        {
            ...
            //al.Add("gif");
            //al.Add("png");
            string[] al = siteConfig.ImgFormat.Split(',');
            if (al.Contains(_fileExt.ToLower()))
            {
                return true;
            }
        ...
        private bool CheckFileSize(string _fileExt, int _fileSize)
        {
            ...
            if (IsImage(_fileExt))
            {
                if (this.siteConfig.FileSize > 0 && _fileSize > this.siteConfig.FileSize)
                {
                    return false;
                }
            }
